	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf(
					"NewTemplater() error = %v, wantErr %v",
					err,
					tt.wantErr,
				)
				return
...
func TestTemplater_Execute(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf(
					"Templater.Execute() error = %v, wantErr %v",
					err,
					tt.wantErr,
				)
				return
...
func TestTemplater_GenerateTagsAndDate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.want {
				t.Errorf(
					"Templater.GenerateTagsAndDate() got = %v, want %v",
					got,
					tt.want,
				)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf(
					"Templater.GenerateTagsAndDate() got1 = %v, want %v",
					got1,
					tt.want1,
				)
			}
