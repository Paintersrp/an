...
func TestInitialPrompt(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := InitialPrompt(tt.args.cfgPath); !reflect.DeepEqual(
				got,
				tt.want,
			) {
				t.Errorf(
					"InitialPrompt() = %v, want %v",
					got,
					tt.want,
				)
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf(
					"InitPromptModel.Update() got = %v, want %v",
					got,
					tt.want,
				)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf(
					"InitPromptModel.Update() got1 = %v, want %v",
					got1,
					tt.want1,
				)
			}
...
func TestInitPromptModel_updateInputs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.updateInputs(tt.args.msg); !reflect.DeepEqual(
				got,
				tt.want,
			) {
				t.Errorf(
					"InitPromptModel.updateInputs() = %v, want %v",
					got,
					tt.want,
				)
			}
...
func TestSetupDefaults(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SetupDefaults(tt.args.homePath); !reflect.DeepEqual(
				got,
				tt.want,
			) {
				t.Errorf(
					"SetupDefaults() = %v, want %v",
					got,
					tt.want,
				)
			}
